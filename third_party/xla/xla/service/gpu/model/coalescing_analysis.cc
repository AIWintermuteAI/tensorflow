...
std::vector<Interval> ExtendIntervals(const std::vector<Interval>& intervals,
  ...
  for (const Interval& interval : intervals) {
    extended_intervals.emplace_back(interval.lower, interval.upper + length);
  }
  // Compute union of overlapped intervals.
  std::vector<Interval> overlapped_intervals;
  for (int i = 0; i < extended_intervals.size();) {
    int64_t lower = extended_intervals[i].lower;
    int64_t upper = extended_intervals[i].upper;
    ++i;
    while (i < extended_intervals.size() &&
           upper >= extended_intervals[i].lower - 1) {
      upper = std::max(upper, extended_intervals[i].upper);
      ++i;
    }
